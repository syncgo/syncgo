...
func TestElasticsearchSync_ReplicatesRowsFromPostgres(t *testing.T) {
	...
	setupTable(t, ctx, pgCfg, syncElasticTable, cfg.PostgreSQL.PublicationName)
	...
	insertRow(t, ctx, pgCfg, syncElasticTable, 1, "foo")
	insertRow(t, ctx, pgCfg, syncElasticTable, 2, "bar")
	insertRow(t, ctx, pgCfg, syncElasticTable, 3, "baz")
	...
	updateRow(t, ctx, pgCfg, syncElasticTable, 1, "foo-updated")
	assertDocument(t, cfg.SearchEngine.Address, cfg.SearchEngine.Index, "1", "foo-updated")

	deleteRow(t, ctx, pgCfg, syncElasticTable, 2)
